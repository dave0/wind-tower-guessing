...
my %metro_areas = (
	...
	gta   => [qw(
		...
		TORONTON BURLIGNTON GEOGRETOWN BRULINGTON MAKRHAM TOROTNO MARKAM RICHMOND
		),
		# Can't qw() these:
...
sub guess_metro_area
{
	...
	# Special cases first
	if( $station->{Link_Station_Location} && (
		$station->{Station_Location} =~ /^(gatineau|aylmer|hull)/i ||
		$station->{Link_Station_Location} =~ /^(gatineau|aylmer|hull)/i ) ) {
		return 'Gatineau';
	}
